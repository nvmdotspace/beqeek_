/**
 * Connector Select Page (Compact Design)
 *
 * Connector type catalog with redesigned compact cards
 * - 2-column grid (not 3) prevents overcrowding
 * - 88px card height (45% reduction from original)
 * - Improved scannability with horizontal layout
 *
 * Design: plans/251117-connector-card-redesign/
 */
...
import { ConnectorCardCompact } from '../components/connector-card-compact';
...
export function ConnectorSelectPageCompact() {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      {/* Grid - 2 columns max (not 3) for better readability with compact cards */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filteredTypes.map((type) => (
            <ConnectorCardCompact
              key={type.type}
              connectorType={type}
              onClick={() => handleCardClick(type.type, type.name)}
            />
          ))}
        </div>
      ) : (
