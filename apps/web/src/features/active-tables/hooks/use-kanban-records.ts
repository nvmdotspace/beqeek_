 ...
 * Kanban Records Hook
 *
 * Separate API call for Kanban view with:
 * - Default pageSize: 100 (no infinite scroll in kanban)
 * - Automatic refetch when kanban config changes
 * - Status field filtering support
 *
...
import type { TableRecord, Table, KanbanConfig } from '@workspace/active-tables-core';
import type { ActiveRecordsResponse } from '../types';

const KANBAN_DEFAULT_PAGE_SIZE = 100;
...
/**
 * Hook for fetching Kanban board records
 *
 * Unlike useInfiniteActiveTableRecords, this hook:
 * - Fetches all records in one call (default 100)
 * - Uses kanbanScreenId as part of query key for separate caching
 * - Optimized for board view (no infinite scroll)
 */
export function useKanbanRecords(
  ...
  options?: UseKanbanRecordsOptions,
) {
  const {
    pageSize = KANBAN_DEFAULT_PAGE_SIZE,
    direction = 'desc',
    enabled = true,
    encryptionKey,
    filters,
  ...
  // Query key includes kanbanScreenId for separate caching per board
  const queryKey = useMemo(
    ...
    [workspaceId, tableId, kanbanConfig?.kanbanScreenId, pageSize, direction, encryptedFilters],
  );
  ...
  const query = useQuery<ActiveRecordsResponse, Error>({
    ...
    enabled: enabled && !!workspaceId && !!tableId && !!kanbanConfig,
    staleTime: 30 * 1000,
    gcTime: 5 * 60 * 1000,
  ...
  useEffect(() => {
    const decryptAllRecords = async () => {
      ...
      try {
        ...
        console.error('[useKanbanRecords] Decryption failed:', error);
