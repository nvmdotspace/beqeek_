...
export function DateField(props: FieldRendererProps) {
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const newValue = e.target.value; // YYYY-MM-DD format
  ...
  if (mode === 'display') {
    ...
    try {
      const date = parse(stringValue, 'yyyy-MM-dd', new Date());
      const formatted = format(date, 'dd/MM/yyyy'); // Vietnamese format
